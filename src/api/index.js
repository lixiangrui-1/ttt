...
// 登录账号
export const submitLogin = (userInfoObj) =>
  axios.post("/login", qs.stringify(userInfoObj));
// 注册账号
export const submitRegister = (registerObj) =>
  axios.post("/users/reg", qs.stringify(registerObj));
// 获取验证码
export const getVerigyCode = (mobile) => {
  return axios.post("/users/get_reg_code", qs.stringify({ mobile }));
};
// 搜索商品
export const searchGoods = (value) => axios.get("/goods/search?cid=" + value);
// 搜索建议查询
export const searchSuggest = (value) =>
  axios.get("/goods/search?query=" + value);
// 获取购物车数据
...
// 创建订单
export const createOrder = (goodsInfo) =>
  axios.post("/my/orders/create", goodsInfo);
// 获取订单
